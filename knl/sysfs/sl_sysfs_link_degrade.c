...
static ssize_t state_show(struct kobject *kobj, struct kobj_attribute *kattr, char *buf)
{
	...
	ctl_link = container_of(kobj, struct sl_ctl_link, degrade_kobj);
	...
	sl_log_dbg(ctl_link, LOG_BLOCK, LOG_NAME, "degrade state show (state = %u %s)",
...
static ssize_t is_rx_degraded_show(struct kobject *kobj, struct kobj_attribute *kattr, char *buf)
{
	...
	sl_log_dbg(ctl_link, LOG_BLOCK, LOG_NAME, "is rx degraded show (degrade = %u %s)",
...
static ssize_t is_tx_degraded_show(struct kobject *kobj, struct kobj_attribute *kattr, char *buf)
{
	...
	sl_log_dbg(ctl_link, LOG_BLOCK, LOG_NAME, "is tx degraded show (degrade = %u %s)",
...
int sl_sysfs_link_degrade_create(struct sl_ctl_link *ctl_link)
{
	...
	sl_log_dbg(ctl_link, LOG_BLOCK, LOG_NAME, "link degrade create (num = %u)", ctl_link->num);

	rtn = kobject_init_and_add(&ctl_link->degrade_kobj, &link_degrade, &ctl_link->kobj, "auto_lane_degrade");
	if (rtn) {
		sl_log_err(ctl_link, LOG_BLOCK, LOG_NAME,
			"link degrade create kobject_init_and_add failed [%d]", rtn);
		kobject_put(&ctl_link->degrade_kobj);
		return rtn;
	}
...
{
	sl_log_dbg(ctl_link, LOG_BLOCK, LOG_NAME, "link degrade delete (num = %u)", ctl_link->num);
