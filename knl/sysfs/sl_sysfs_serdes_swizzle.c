...
int sl_sysfs_serdes_lane_swizzle_create(struct sl_ctl_lgrp *ctl_lgrp, u8 asic_lane_num)
{
	int rtn;

	sl_log_dbg(ctl_lgrp, LOG_BLOCK, LOG_NAME,
		"serdes lane swizzle create (lgrp = 0x%p)", ctl_lgrp);

	rtn = kobject_init_and_add(&(ctl_lgrp->serdes_lane_swizzle_kobjs[asic_lane_num].kobj),
		&serdes_lane_swizzle_info, &(ctl_lgrp->serdes_lane_kobjs[asic_lane_num]), "swizzle");
	if (rtn) {
		sl_log_err(ctl_lgrp, LOG_BLOCK, LOG_NAME,
			"serdes lane swizzle create kobject_init_and_add failed [%d]", rtn);
		kobject_put(&(ctl_lgrp->serdes_lane_swizzle_kobjs[asic_lane_num].kobj));
		return -ENOMEM;
	}
	ctl_lgrp->serdes_lane_swizzle_kobjs[asic_lane_num].ctl_lgrp      = ctl_lgrp;
	ctl_lgrp->serdes_lane_swizzle_kobjs[asic_lane_num].asic_lane_num = asic_lane_num;

	sl_log_dbg(ctl_lgrp, LOG_BLOCK, LOG_NAME,
		"serdes lane swizzle create (serdes_kobj = 0x%p)", &(ctl_lgrp->serdes_kobj));
	return 0;
}

void sl_sysfs_serdes_lane_swizzle_delete(struct sl_ctl_lgrp *ctl_lgrp, u8 asic_lane_num)
{
	sl_log_dbg(ctl_lgrp, LOG_BLOCK, LOG_NAME,
		"serdes lane swizzle delete (lgrp = 0x%p)", ctl_lgrp);

	kobject_put(&(ctl_lgrp->serdes_lane_swizzle_kobjs[asic_lane_num].kobj));
	ctl_lgrp->serdes_lane_swizzle_kobjs[asic_lane_num].ctl_lgrp = NULL;
}
