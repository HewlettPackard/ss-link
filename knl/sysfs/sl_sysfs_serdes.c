...
{
	switch (ctl_lgrp->config.furcation) {
...
int sl_sysfs_serdes_create(struct sl_ctl_lgrp *ctl_lgrp)
{
	...
	sl_log_dbg(ctl_lgrp, LOG_BLOCK, LOG_NAME, "serdes create (lgrp = 0x%p)", ctl_lgrp);

	if (!ctl_lgrp->parent_kobj) {
		sl_log_err(ctl_lgrp, LOG_BLOCK, LOG_NAME, "serdes create missing parent");
		return -EBADRQC;
	}

	rtn = kobject_init_and_add(&(ctl_lgrp->serdes_kobj),
		&serdes_info, ctl_lgrp->parent_kobj, "serdes");
	if (rtn) {
	...
	for (asic_lane_num = 0; asic_lane_num < SL_ASIC_MAX_LANES; ++asic_lane_num) {
		...
		rtn = sl_sysfs_serdes_lane_state_create(ctl_lgrp, asic_lane_num);
		if (rtn) {
			sl_log_err(ctl_lgrp, LOG_BLOCK, LOG_NAME,
		...
		rtn = sl_sysfs_serdes_lane_eye_create(ctl_lgrp, asic_lane_num);
		if (rtn) {
			sl_log_err(ctl_lgrp, LOG_BLOCK, LOG_NAME,
	...
	sl_log_dbg(ctl_lgrp, LOG_BLOCK, LOG_NAME,
		"serdes create (serdes_kobj = 0x%p)", &(ctl_lgrp->serdes_kobj));
	return 0;
...
void sl_sysfs_serdes_delete(struct sl_ctl_lgrp *ctl_lgrp)
{
	...
	sl_log_dbg(ctl_lgrp, LOG_BLOCK, LOG_NAME, "serdes delete (lgrp = 0x%p)", ctl_lgrp);
