...
int sl_sysfs_lgrp_policy_create(struct sl_ctl_lgrp *ctl_lgrp)
{
	...
	sl_log_dbg(ctl_lgrp, LOG_BLOCK, LOG_NAME, "lgrp policies create (lgrp = 0x%p)", ctl_lgrp);

	rtn = kobject_init_and_add(&(ctl_lgrp->policy_kobj), &policy_info, ctl_lgrp->parent_kobj, "policies");
	if (rtn) {
		sl_log_err(ctl_lgrp, LOG_BLOCK, LOG_NAME, "lgrp policies create failed [%d]", rtn);
...
{
	sl_log_dbg(ctl_lgrp, LOG_BLOCK, LOG_NAME, "lgrp policies delete (lgrp = 0x%p)", ctl_lgrp);
