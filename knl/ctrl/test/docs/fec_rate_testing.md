...
Once you have transferred the test scripts from `knl/ctrl/test/scripts` onto the emulator. You can begin testing. The *full suite* of test runs rather quickly and should complete in roughly 40 seconds. Any longer, it is likely a test has failed.
...
```sh
# Example of transferring scripts to z1build to the sshotplat users home directory in `ctrl_tests`
rsync -az --progress sl-driver/knl/ctrl/test/scripts sshotplat@z1build:~/ctrl_tests
```
...
export CTRL_TEST_DIR="/root/sshotplat/ctrl_tests/scripts"
grep -n result $CTRL_TEST_DIR/ctrl_tests.log
```
...
read: # ddebug file sl_ctrl_test.c on
read: # ddebug file sl_ctrl_link_fec.c on
...
```sh
[10602.168105] sl [0:00:0]^ lgrp00         ctrl-l-fec     : UCW rate reached down limit (UCW = 60 CCW = 0)
[10602.175931] sl [0:01:0]^ lgrp01         ctrl-l-fec     : UCW rate reached down limit (UCW = 60 CCW = 0)
```
...
Example output from dmesg.
```sh
[10821.624081] sl [0:01:0]^ lgrp01         ctrl-l-fec     : CCW rate reached down limit (UCW = 0 CCW = 20000000)
[10821.630561] sl [0:00:0]^ lgrp00         ctrl-l-fec     : CCW rate reached down limit (UCW = 0 CCW = 20000000)
```
