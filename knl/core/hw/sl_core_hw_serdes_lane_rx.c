...
int sl_core_hw_serdes_lane_up_rx_setup(struct sl_core_lgrp *core_lgrp, u8 serdes_lane_num)
{
	...
	SL_CORE_HW_PMI_WR(core_lgrp, core_lgrp->serdes.dt.dev_id, serdes_lane_num,
		0, 0xD1A7, 0, 4, 0x0010); /* rx_clk_vld_frc_val */
	SL_CORE_HW_PMI_WR(core_lgrp, core_lgrp->serdes.dt.dev_id, serdes_lane_num,
		0, 0xD1A7, 0, 3, 0x0008); /* rx_clk_vld_frc */
	SL_CORE_HW_PMI_WR(core_lgrp, core_lgrp->serdes.dt.dev_id, serdes_lane_num,
		0, 0xD1A7, 0, 0, 0x0001); /* rx_s_clkgate_frc_on */

	/* power on */
	SL_CORE_HW_PMI_WR(core_lgrp, core_lgrp->serdes.dt.dev_id, serdes_lane_num,
		0, 0xD1A1, 0, 0, 0x0001); /* rx_s_pwrdn */
	...
	/* disable DP */
	SL_CORE_HW_PMI_WR(core_lgrp, core_lgrp->serdes.dt.dev_id, serdes_lane_num,
		0, 0xD1C1, 0, 0, 0x0001); /* RX_LN_DP_EN */
	...
	struct sl_core_link *core_link, u8 serdes_lane_num)
{
	...
	SL_CORE_HW_PMI_WR(core_lgrp, core_lgrp->serdes.dt.dev_id, serdes_lane_num,
		...
		0, 0xFFFF); /* LANE_CFG_FWAPI_DATA0 */

	/* invert */
	SL_CORE_HW_PMI_WR(core_lgrp, core_lgrp->serdes.dt.dev_id, serdes_lane_num,
		0, 0xD163, core_lgrp->serdes.dt.lane_info[asic_lane_num].rx_invert,
		0, 0x0001); /* RX_PMD_DP_INVERT */
	...
	if (is_flag_set(core_link->config.flags, SL_LINK_CONFIG_OPT_SERDES_LOOPBACK_ENABLE)) {
		...
		SL_CORE_HW_PMI_WR(core_lgrp, core_lgrp->serdes.dt.dev_id, serdes_lane_num,
			0, 0xD162, 1, 0, 0x0001); /* DIG_LPBK_EN */
	} else {
		sl_core_log_dbg(core_lgrp, LOG_NAME,
			"lane up rx config loopback off (logical = %u)", serdes_lane_num);
		SL_CORE_HW_PMI_WR(core_lgrp, core_lgrp->serdes.dt.dev_id, serdes_lane_num,
			0, 0xD162, 0, 0, 0x0001); /* DIG_LPBK_EN */
	}
...
int sl_core_hw_serdes_lane_up_rx_start(struct sl_core_lgrp *core_lgrp, u8 serdes_lane_num)
{
	...
	/* enable DP */
	SL_CORE_HW_PMI_WR(core_lgrp, core_lgrp->serdes.dt.dev_id, serdes_lane_num,
		0, 0xD1C1, 1, 0, 0x0001); /* RX_LN_DP_EN */
...
void sl_core_hw_serdes_lane_down_rx_stop(struct sl_core_lgrp *core_lgrp, u8 serdes_lane_num)
{
	...
	/* power down */
	SL_CORE_HW_PMI_WR(core_lgrp, core_lgrp->serdes.dt.dev_id, serdes_lane_num,
		0, 0xD1A1, 1, 0, 0x0001); /* rx_s_pwrdn */

	usleep_range(10000, 20000); /* avoid power surge */

	/* clear the config */
	SL_CORE_HW_PMI_WR(core_lgrp, core_lgrp->serdes.dt.dev_id, serdes_lane_num,
		0, 0xD1AD, 0, 0, 0xFFFF); /* LANE_CFG_FWAPI_DATA0 */
