...
static ssize_t rx_lane_map_show(struct kobject *kobj, struct kobj_attribute *kattr, char *buf)
{
	...
	rtn = sl_core_data_link_degrade_rx_lane_map_get(core_link, &rx_lane_map);
	if (rtn == -EBADRQC)
		return scnprintf(buf, PAGE_SIZE, "%s\n", "inactive");
	if (rtn)
		return scnprintf(buf, PAGE_SIZE, "error\n");
	...
	sl_log_dbg(core_link, LOG_BLOCK, LOG_NAME,
		   "rx degrade lane map show (rx_lane_map = 0x%X)", rx_lane_map);
...
static ssize_t tx_lane_map_show(struct kobject *kobj, struct kobj_attribute *kattr, char *buf)
{
	...
	rtn = sl_core_data_link_degrade_tx_lane_map_get(core_link, &tx_lane_map);
	if (rtn == -EBADRQC)
		return scnprintf(buf, PAGE_SIZE, "%s\n", "inactive");
	if (rtn)
		return scnprintf(buf, PAGE_SIZE, "error\n");
	...
	sl_log_dbg(core_link, LOG_BLOCK, LOG_NAME,
		   "tx degrade lane map show (tx_lane_map = 0x%X)", tx_lane_map);
