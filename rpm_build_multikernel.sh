...
# zypper dutifully gathers up the build requirements of that RPM.
    ...
    if [ -n "$ZYPPER_COMMAND" ]; then
        ...
        echo "rpmbuild -ts ./WORKSPACE/SOURCES/$SOURCE --target \"$ARCH\" --define \"_os $OS\" --define \"_arch $ARCH\" --define \"_buildname $BUILDNAME\" --define \"_git_hash $GIT_HASH\" " > WORKSPACE/deps-helper.sh
...
for kver in $kernels; do
    ...
    echo "rpmbuild -ba ./WORKSPACE/SPECS/$SPECFILE_BASENAME --define \"_topdir $TOPDIR\" --define \"kernel_version $kver\" --target \"$ARCH\" --define \"_os $OS\" --define \"_arch $ARCH\" --define \"_buildname $BUILDNAME\" --define \"debug_package $DEBUG_PACKAGE\" --define \"_git_hash $GIT_HASH\" " > WORKSPACE/helper.sh
