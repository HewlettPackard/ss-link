...
brief="Test auto-negotiation selects fastest speed: ck100 and cd50 enabled (should select ck100g)."

source "${SL_TEST_DIR}/sl_test_env.sh"

settings="${SL_TEST_DIR}/systems/settings/ck100_x1.sh"
autoneg=1
hpe_map_linktrain_set=1

LINK_NOTIF_TIMEOUT=275000 # Timeout in milliseconds
expected_speed="ck100g"
...
function main {
	...
	sl_test_info_log "${FUNCNAME}" \
		"lgrp_setup (ldev_num = ${ldev_num}, lgrp_nums = (${lgrp_nums[*]}), tech_map_ck100g=${tech_map_ck100g_set} cd50g=${tech_map_cd50g_set})"
