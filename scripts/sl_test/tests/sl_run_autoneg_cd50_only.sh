...
brief="Test auto-negotiation selects fastest speed: cd50 only enabled (should select cd50)."
...
function main {
	...
	sl_test_info_log "${FUNCNAME}" \
		"lgrp_setup (ldev_num = ${ldev_num}, lgrp_nums = (${lgrp_nums[*]}), tech_map_cd50g=${tech_map_cd50g_set})"
