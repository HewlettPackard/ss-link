...
brief="Test link-up notification is received when cabled links are commanded up with loopback host enabled for the transceiver."
...
usage="Usage: ${SCRIPT_NAME} [-h | --help] [-m | --max_num_active_media] [-b | --brief] [-g | --lgrp_nums]"
description=$(cat <<-EOF
${brief}

Options:
-b, --brief                Brief test description.
-m, --max_num_active_media Automatically discover link groups on active media and test no more than the specified number.
-g, --lgrp_nums            Link group numbers to test.
-h, --help                 This message.
EOF
)

options=$(getopt -o "hm:g:b" --long "help,max_num_active_media:,lgrp_nums:,brief" -- "$@")
...
while true; do
	case "$1" in
		...
		-g | --lgrp_nums)
			...
			shift 2
			;;
		-m | --max_num_active_media)
			__sl_test_media_active_map_get ${ldev_num} lgrp_nums
			shift 2
			;;
		-b | --brief)
			echo ${brief}
