...
brief="Test the FEC UCW limit during link up check exceeds and sends a link-up-fail notification with cause \
matching last down cause in sysfs."
...
function test_verify {
	...
	for lgrp_num in "${lgrp_nums[@]}"; do
		...
		for link_num in "${sl_test_link_nums[@]}"; do
			...
			sysfs_down_causes=($(cat ${lgrp_sysfs}/${lgrp_num}/test_port/${link_num}/link/last_down/cause_map_0))
			...
			if [[ "${found}" != true ]]; then
				...
				sl_test_info_log "${FUNCNAME}" \
					"${notif_type} (ldev_num = ${ldev_num}, lgrp_num = ${lgrp_num}, link_num = ${link_num})"
...
function main {
	...
	sl_test_lgrp_links_notif_wait ${ldev_num} "${lgrp_nums[*]}" "link-ucw-warn" ${LINK_NOTIF_TIMEOUT} sl_test_notifs
	rtn=$?
	if [[ "${rtn}" != 0 ]]; then
		sl_test_error_log "${FUNCNAME}" "lgrp_links_notif_wait failed [${rtn}]"
		return ${rtn}
