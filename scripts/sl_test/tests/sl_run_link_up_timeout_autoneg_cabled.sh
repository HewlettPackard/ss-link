...
brief="Test link-up-fail notification is received when cabled links are commanded up with autoneg enabled, but timeout early."
		...
		for link_num in "${sl_test_link_nums[@]}"; do
			...
			for notif in "${notifs[@]}"; do
				...
				if [[ "${notif_type}" == "link-up-fail" && \
					"${notif_fields[7]}" == "up-tries" && \
					"${notif_fields[8]}" == "timeout" && \
					"${notif_fields[9]}" == "retryable" && \
					"${notif_fields[10]}" == "origin-up" ]]; then
			...
			if [[ "${found}" != true ]]; then
				sl_test_error_log "${FUNCNAME}" \
					"Expected: \"link-up-fail\", Found: \"${notif_type}\" (ldev_num = ${ldev_num}, lgrp_num = ${lgrp_num}, link_num = ${link_num})"
									return 1
			fi
