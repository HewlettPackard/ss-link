...
brief="Test auto-negotiation selects fastest speed: bj100 and ck100 enabled (should select bj100g)."

source "${SL_TEST_DIR}/sl_test_env.sh"

settings="${SL_TEST_DIR}/systems/settings/bj100_x1.sh"

LINK_NOTIF_TIMEOUT=275000 # Timeout in milliseconds
expected_speed="bj100g"
...
tech_map_bj100g_set=1
tech_map_ck100g_set=1
...
function main {
	...
	sl_test_info_log "${FUNCNAME}" \
		"lgrp_setup (ldev_num = ${ldev_num}, lgrp_nums = (${lgrp_nums[*]}), tech_map_bj100g=${tech_map_bj100g_set} ck100g=${tech_map_ck100g_set})"
